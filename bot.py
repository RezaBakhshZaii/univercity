...
async def taidie(c, m):
    ...
    if str(type(user)) == "<class 'tuple'>":
        caption = f"☑️ تاییدیه دروس ترم شما به شماره دانشجویی {user[1]}"
        if exist_photo(f"ok_s{user[1]}.png"):
            await c.send_photo(m.from_user.id, f"ok_s{user[1]}.png", caption=caption)
        else:
...
async def plan_class(c, m):
    user = check_exist(m.from_user.id, "9")
    if str(type(user)) == "<class 'tuple'>":
        caption = f"🗒 برنامه کلاسی دروس ترم شما به شماره دانشجویی {user[1]}"
        if exist_photo(f"class_s{user[1]}.png"):
            await c.send_photo(m.from_user.id, f"class_s{user[1]}.png", caption=caption)
        else:
            photos = get_photos(user[1])
    ...
    if str(type(user)) == "<class 'tuple'>":
        if exist_photo(f"presence_s{user[1]}.png"):
            await c.send_photo(m.from_user.id, f"presence_s{user[1]}.png",
                               caption=f"🧾 حضور غیاب کلاسی دروس ترم شما به شماره دانشجویی {user[1]}")
        else:
            photos = get_photos(user[1])
    ...
    if str(type(user)) == "<class 'tuple'>":
        if exist_photo(f"numbers_s{user[1]}.png"):
            await c.send_photo(m.from_user.id, f"numbers_s{user[1]}.png",
                               caption=f"🗂 نمرات ترم شما به شماره دانشجویی {user[1]}")
        else:
            photos = get_photos(user[1])
    ...
    if str(type(user)) == "<class 'tuple'>":
        if exist_photo(f"exam_s{user[1]}.png"):
            await c.send_photo(m.from_user.id, f"exam_s{user[1]}.png",
                               caption=f"📝 برنامه امتحانی دروس ترم شما به شماره دانشجویی {user[1]}")
        else:
            photos = get_photos(user[1])
...
1️⃣ بخش مشخصات فردی : این بخش یه سری مشخصات اصلی و مهم قسمت مشخصات فردی پرتالت رو بهتون نشون میده 

2️⃣ بخش دروس ترم ( تاییدیه ) : این بخش هم دقیقا همون قسمت تایبدیه پرتال هستش که باهاش میتونین دروس انتخاب شده ، حذف شده ، رزرو شده و... رو ببینین و مطلع باشین

3️⃣ بخش برنامه کلاسی : این بخش هم برنامه کلاسیتون رو در اختیارتون میزاره تا از سر زدن به پرتال بی‌نیاز باشین

4️⃣ بخش حضور و غیاب : این بخش شما رو از وضعیت حضور در کلاس و غیبت های ثبت شده مطلع می‌کنه تا از برگزاری کلاس و وضعیت فعلی تون با خبر بشین

5️⃣ بخش برنامه امتحانی : این بخش هم برنامه امتحانیتون رو در اختیارتون میزاره تا از سر زدن به پرتال بی‌نیاز باشین 

6️⃣ بخش نمرات ترم : این بخش هم نمرات ترم شما رو نمایش میده ، البته تا قبل از امتحانات پایان ترم نمرات ترم گذشته رو نمایش میده به انتهای ترم هم که نزدیک بشیم نمرات ثبت شده خود ترم رو میبینین

7️⃣ بخش همگام‌سازی : بدلیل بهینگی در استفاده از منابع، اطلاعات پرتال شما بعد از ورودتون ذخیره میشن و از مشغول شدن لحظه‌ای برای دریافت اطلاعات از سرور جلوگیری میشه ، اگر احساس کردین اطلاعات پرتالتون تغییری داشته و باید این تغییرات رو ببینید همگام‌سازی رو بزنین تا اطلاعات با دریافت مجدد از پرتال همگام سازی بشن
...
@app.on_message(filters.private & filters.regex("^✅ همگام‌سازی پرتال$"))
async def hamgam_portal(c, m):
    t = await m.reply("📊")
    w = await m.reply("🔄 در حال به‌همگام‌سازی اطلاعات پرتال... لطفاً اندکی منتظر بمانید🙏")
    user = get_information_id(m.from_user.id)
    delete_photo(user[1])
    delete_student(user[1])
    result_login = await arsnic.login(user[1], user[2], "all", m.from_user.id, c)
    if result_login == 1:
        user = check_exist(m.from_user.id, "9")
        if not exist_photo(f"image_s{user[1]}.png"):
            to_file(user[9], f"image_s{user[1]}.png")
        await app.send_photo(m.from_user.id, f"image_s{user[1]}.png",
                             caption=f"""🫡سلام {user[3][user[3].find(':') + 1:]} عزیز  \n 
✅ همگام‌سازی پرتال شما با موفقیت انجام شد 

🔄 آخرین تاریخ همگام‌سازی اطلاعات : 
📆 {user[10].split()[0]}
⏰ {user[10].split()[1]}""")
    elif result_login == 0:
        await m.reply("errors ... ", reply_markup=keyboard_home)
    elif result_login == 2:
        await m.reply("🏠")
        await m.reply("✅به منو اصلی بازگشتید", reply_markup=keyboard_home)
    else:
        await m.reply(result_login, reply_markup=keyboard_home)
    await t.delete()
    await w.delete()
